...
7. [Phase 6 — Quarto and Pyodide](#7-phase-6--quarto-and-pyodide)
8. [Architecture principles](#8-architecture-principles)
9. [Open questions and future directions](#9-open-questions-and-future-directions)
...
## 7. Phase 6 — Quarto and Pyodide

Wayward House is a computational geography essay series. Before content development begins, a platform gate must clear: Quarto rendering and Pyodide in-browser execution must be live and end-to-end tested.

### Three-channel model

Each essay ships as three coordinated artefacts from a single `.qmd` source:

| Channel | Artefact | Audience |
|---|---|---|
| 1 | Jekyll essay (rendered Markdown in `_posts/`) | Web readers |
| 2 | Pyodide cells — live Python in the browser | Interactive learners |
| 3 | Jupyter notebook exported from same `.qmd` | Local/Pi execution |

The source file (`essay.qmd`) is the canonical version. Channels 1 and 3 are generated by `quarto render` and `quarto convert`; Channel 2 is authored inline using div fence syntax and rendered by the Pyodide adapter at page load.

### `.pyodide-cell` div fence convention

Pyodide cells use Quarto's div fence syntax, which passes through to Markdown output as a `<div class="pyodide-cell">` regardless of Quarto version:

```markdown
::: {.pyodide-cell}
```python
import numpy as np
I0 = 1000; k = 0.5; LAI = 3.0
print(f"{I0 * np.exp(-k * LAI):.1f} W/m²")
```
:::
```

Standard `{python}` blocks with `eval: false` appear in the essay as read-only code and are exported to the notebook as executable cells:
...
### `pyodide: true` front matter flag

Setting `pyodide: true` in post front matter causes `_layouts/default.html` to add `tag-hash-pyodide` to the `<body>` class list. The viz registry's Pyodide entry detects this flag (or falls back to detecting `.pyodide-cell` elements directly) and triggers the Pyodide load sequence.

### Pyodide adapter (`assets/js/viz/pyodide.js`)

The adapter is the last entry in `viz-registry.js`. Placement matters: Pyodide is ~8 MB from CDN, and the registry is processed sequentially. Putting it last means all other viz adapters (KaTeX, ECharts, Leaflet, etc.) complete before the heavyweight load begins.

The adapter exports two functions:

- **`initPyodide()`** — dynamic-imports `pyodide.mjs` from jsDelivr, calls `loadPyodide()`, pre-loads `numpy`, stores the singleton in module scope
- **`renderPyodideCell(el)`** — transforms a `.pyodide-cell` div into an interactive widget with a Run button, status indicator, and output pane

### Pyodide constraints (platform gate scope)

| Constraint | Reason |
|---|---|
| `numpy` only (pre-loaded) | Additional packages require explicit `loadPackage()` — not wired in platform gate |
| No file I/O | WASM sandbox restriction |
| Cells are stateless | Each Run executes the cell's source from scratch; no shared Python globals between cells |
| Text output only | `stdout`/`stderr` captured via `setStdout`/`setStderr`; no matplotlib figure rendering in gate |

These constraints are for the platform gate. Subsequent waves can add `pandas`, `scipy`, and figure rendering by extending `initPyodide()` and `renderPyodideCell()`.

### Quarto render script

`scripts/quarto-render.sh` is a generic helper for rendering any essay:
...
The script: renders `.qmd` → `essay.md` (Jekyll-md format), exports to `notebook.ipynb`, rsyncs figures to the site's `assets/images/quarto/<slug>/`, and rewrites figure paths in the Markdown. The author then moves `essay.md` to `_posts/` with the correct date prefix.

### Content repository

`computational-geography-lab/` lives alongside the site repos under `~/src/websites/`. It is not a git submodule — it is an independent repo (GitHub org: `waywardhouse/computational-geography-lab`) that acts as the authoring workspace. Essays flow one-way from here into the site repo's `_posts/`.

```
computational-geography-lab/
├── _quarto.yml              ← project-wide: jekyll-md format, numpy, python3 kernel
├── requirements.txt         ← numpy, pandas, matplotlib, scipy, jupyter
├── essays/
│   └── A1-what-is-a-spatial-model/
│       ├── essay.qmd        ← canonical source
│       ├── notebook.ipynb   ← Channel 3 export (committed)
│       └── figures/         ← Quarto-generated (gitignored)
├── shared/
│   └── plot_style.py        ← matplotlib defaults
└── data/
    └── README.md
```

> **Decision point: separate content repo over `_posts/` authoring**
> Authoring `.qmd` directly in a site repo's `_posts/` would require Quarto to be part of the site CI pipeline and would mix the computational authoring environment with the static site build. Keeping the content repo separate means the authoring environment (Python venv, Quarto, Jupyter) is isolated from the Jekyll/Vite build. Essays are compiled artefacts that flow into the site, not sources that the site builds from.
...
*Last updated: February 2026 (Phase 6 — Quarto + Pyodide platform gate). See `ROADMAP.md` for the current backlog.*
